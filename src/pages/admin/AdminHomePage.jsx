...
import {
  Users, User, BookOpen, ClipboardList, PlusCircle
} from "lucide-react";
import {
  StatCard,
  QuickActionButton,
      ...
      <div className="flex min-h-screen bg-gray-50">
        ...
        <main className="flex-1 p-6 space-y-6">
          ...
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-2xl font-semibold text-gray-800">
                {t("welcome")}, {userName?.split(" ")[0] || t("admin")}
              </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <StatCard icon={<Users className="h-6 w-6 text-blue-500" />} value={counts.teachers} label={t('teachers')} link="/admin/teachers" color="bg-blue-50" />
            <StatCard icon={<User className="h-6 w-6 text-green-500" />} value={counts.students} label={t('students')} link="/admin/students" color="bg-green-50" />
            <StatCard icon={<BookOpen className="h-6 w-6 text-orange-500" />} value={counts.lessons} label={t('lessons')} link="/admin/lessonlog" color="bg-orange-50" />
            <StatCard icon={<ClipboardList className="h-6 w-6 text-purple-500" />} value={counts.exams} label={t('exams')} link="/admin/exams" color="bg-purple-50" />
          </div>
