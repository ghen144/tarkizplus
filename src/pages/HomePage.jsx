...
import {
    collection, getDocs, addDoc, deleteDoc, doc, query, where, Timestamp, updateDoc, getDoc,
} from "firebase/firestore";
import {useTranslation} from 'react-i18next';

import {User, Users, Calendar, Pin, PinOff} from "lucide-react";


const RAW_DAYS_OF_WEEK = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
const HOURS_RANGE = [13, 14, 15, 16, 17, 18, 19, 20];

const iconClass = "inline h-6 w-6 stroke-[2.25] text-muted-foreground"; // bigger + bolder

const SESSION_ICONS = {
    Group: <Users className={iconClass}/>, Private: <User className={iconClass}/>,
};
...
function NotesPanel({
    ...
    const handlePinNote = async (noteId) => {
        ...
        setNotes((notes) => notes.map((n) => (n.id === noteId ? {...n, pinned: !n.pinned} : n)));
    };

    return (<div
            className={`
        relative transition-all duration-300 shadow-2xl rounded-2xl
        ${collapsed ? "w-14 min-w-[56px] px-0 py-4 bg-gradient-to-b from-blue-200 to-blue-50" : "w-80 px-6 py-6 bg-blue-100"}
      ...
      `}
        ...
        >
            ...
            {collapsed ? (<button
                ...
                >
                    <span
                        className="text-2xl text-blue-600 font-extrabold group-hover:scale-125 transition">{'»'}</span>
                    <span
                        className="mt-2 text-[16px] font-bold text-blue-700 tracking-wide group-hover:scale-110 transition">your notes</span>
            ...
            {/* Notes form and list */}
            {!collapsed && (<>
                    {showNoteForm && (<div className="flex flex-col space-y-3 mb-4 text-sm animate-fadeIn">
                            <select
                    ...
                    <div className="space-y-2 overflow-auto flex-1 pr-1">
                        {sortedNotes.length === 0 && (<div
                                className="flex flex-col items-center justify-center h-full text-gray-400 mt-8 animate-fadeIn">
                                <span className="text-4xl mb-2">📝</span>
                                <span>{t("no_notes")}</span>
                            </div>)}
                        {sortedNotes.map(note => (<div
                                key={note.id}
                            ...
                            >
                                ...
                                <div className="flex flex-col gap-2 items-end">
                                    <button
                                        ...
                                        className="absolute top-2 right-2 text-yellow-400 hover:text-yellow-600 transition"
                                        title={note.pinned ? t('Unpin') : t('Pin')}
                                    >
                                        {note.pinned ? (<Pin className="h-5 w-5 stroke-[2] animate-bounce"/>) : (
                                            <PinOff className="h-5 w-5 stroke-[2]"/>)}
                                    </button>
                                    <button
...
const HomePage = () => {
    ...
    useEffect(() => {
        const fetchData = async (user) => {
            try {
                const teachersSnap = await getDocs(query(collection(db, "teachers"), where("email", "==", user.email)));
                ...
                const scheduleQuery = query(collection(db, "weekly_schedule"), where("teacher", "==", teacherDocName));
                const scheduleSnap = await getDocs(scheduleQuery);
                setWeeklySchedules(scheduleSnap.docs.map(d => ({id: d.id, ...d.data()})));

                const notesSnap = await getDocs(query(collection(db, "notes"), where("teacher_id", "==", teacherDocId)));
                setNotes(notesSnap.docs.map(n => ({id: n.id, ...n.data()})));

                const allStudentsSnap = await getDocs(collection(db, "students"));
                const everyStudent = allStudentsSnap.docs.map(s => ({
                    id: s.id, name: s.data().name,
                }));
    ...
    const handleShowStudents = (slot) => {
        const matched = allStudents.filter((stu) => slot.students?.includes(stu.id));
    ...
    const handleAddNote = async () => {
        ...
        const note = {
            content: newNote, date: noteDate, teacher_id: teacherId, created_at: Timestamp.now(),
        };
        if (selectedStudent) {
    ...
    return (
        ...
        <div
        className="flex min-h-screen bg-gray-50 p-8 gap-6 font-sans">
            ...
            <div className={`flex-1 transition-all duration-300 ${notesCollapsed ? "mr-24" : "mr-0"}`}>
                ...
                <div className="flex items-center gap-6 mb-4 ml-2">
                    <div className="flex items-center gap-1 text-blue-900">
                        ...
                        <span
                            className="bg-blue-100 px-3 py-1 rounded-lg text-sm font-semibold drop-shadow-sm">Group</span>
                    </div>
                    <div className="flex items-center gap-1 text-green-900">
                        <span className="text-xl">{SESSION_ICONS.Private}</span>
                        <span
                            className="bg-green-100 px-3 py-1 rounded-lg text-sm font-semibold drop-shadow-sm">Private</span>
                    </div>
                </div>
                {/* Schedule */}
                <div className="bg-white p-6 shadow-2xl rounded-2xl border border-blue-100">
                    <h2 className="flex items-center gap-2 text-2xl font-bold text-blue-700 mb-4 drop-shadow-sm">
                    ...
                    <div className="overflow-auto">
                        <table className="table-fixed w-full border-collapse text-base">
                            ...
                            <tbody>
                            {HOURS_RANGE.map(hour => (<tr key={hour}>
                          ...
                          `}
                              ...
                              `}
                                            ...
                                            title={t(slot.subject) + "\n" + t(slot.sessionType) + "\n" + (slot.students && slot.students.length > 0 ? slot.students
                                                .map(id => allStudents.find(s => s.id === id)?.name || "")
                                                .join(", ") : t("No students assigned"))}
                                        >
                                                        <span
                                                            className="text-lg">{SESSION_ICONS[slot.sessionType] || "📚"}</span>
                                            <div>
                        ...
                        {showStudentList && selectedSlot && (<div
                            className="fixed inset-0 flex items-center justify-center z-50 bg-black/30 backdrop-blur-sm animate-fadeIn">
                            <div
                                className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-lg relative border border-blue-100 animate-slideIn">
                                <button
                                ...
                                <div className="flex items-center gap-3 mb-2">
                                        <span
                                            className="text-2xl">{SESSION_ICONS[selectedSlot.sessionType] || "📚"}</span>
                                    <span className="text-lg font-bold">{t(selectedSlot.subject)}</span>
                                    <span
                                        className="text-base font-medium text-blue-600 ml-1">({t(selectedSlot.sessionType)})</span>
                                </div>
                    ...
                    <span>
                      ...
                      <strong>{t("Room")}:</strong>{" "}
                                        <span>{selectedSlot.room ||
                                            <span className="text-gray-400 italic">{t("N/A")}</span>}</span>
                    </span>
                                    {/*    <span>*/}
                      ...
                      <strong>{t("Student count")}:</strong>{" "}
                                ...
                                <div className="mt-4">
                                    <strong
                                        className="block mb-1 text-gray-700">{t("Students in this lesson")}:</strong>
                                    {selectedLessonStudents.length > 0 ? (
                                        <ul className="pl-4 list-disc space-y-1 text-base text-gray-900">
                                            {selectedLessonStudents.map(s => (<li key={s.id}>
