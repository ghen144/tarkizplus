...
const HomePage = () => {
  ...
  const handleSaveLesson = async () => {
    ...
    try {
      await addDoc(collection(db, "lessonlog"), {
        teacher_id: teacherId,
        student_ids: selectedSlot.student_ids || [],
      ...
      {selectedSlot && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md space-y-4">
            ...
            <p><strong>{t("time")}:</strong> {selectedSlot.start_time} - {selectedSlot.end_time}</p>
            <p><strong>{t("date")}:</strong> {new Date().toLocaleDateString()}</p>

            <p><strong>{t("class_type")}:</strong> {t(selectedSlot.class_type)}</p>

            <div>
              <label className="font-semibold block mb-1">{t("mark_absent_students")}</label>
            ...
            <div>
              <label className="font-semibold block mb-1">{t("progress_evaluation")}</label>
              <input
                type="text"
