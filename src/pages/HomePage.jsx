    ...
    useEffect(() => {
        const fetchData = async (user) => {
            try {
                ...
                setUserName(teacherDoc.data().name)
        ...
        <div className="p-6">
                            ...
                            return (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <div
                                            className="text-sm text-gray-700 mt-1">{slot.start_time} - {slot.end_time}</div>
                                            ...
                                            return (
                                                <td key={day} className="border px-2 py-1 text-center align-top h-12">
                                                    ...
                                                    {selectedSlot?.id === slot?.id && showStudentList && selectedLessonStudents.length > 0 && (
                                                        <div
                                                            className="mt-2 text-xs text-left bg-white border rounded p-2 shadow">
            ...
            {showModal && selectedSlot && (
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div
                        className=" bg-white rounded-lg p-6 w-11/12 max-w-3xl  max-h-[90vh] overflow-y-auto  space-y-4 "
                    >
                        ...
                        <div className="flex gap-4">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex gap-2">
                                    {/* Hour */}
                                    <select
                                        value={selectedSlot.start_time.split(":")[0]}
                                        onChange={e => {
                                            const newH = e.target.value;
                                            const oldM = selectedSlot.start_time.split(":")[1];
                                            const newStart = `${newH}:${oldM}`;
                                            // clamp end_time if it's now before start
                                            const newEnd = selectedSlot.end_time < newStart ? newStart : selectedSlot.end_time;
                                            setSelectedSlot(s => ({...s, start_time: newStart, end_time: newEnd}));
                                        }}
                                    ...
                                    {/* Minute */}
                                    <select
                                        value={selectedSlot.start_time.split(":")[1]}
                                        onChange={e => {
                                            const newM = e.target.value;
                                            const oldH = selectedSlot.start_time.split(":")[0];
                                            const newStart = `${oldH}:${newM}`;
                                            const newEnd = selectedSlot.end_time < newStart ? newStart : selectedSlot.end_time;
                                            setSelectedSlot(s => ({...s, start_time: newStart, end_time: newEnd}));
                                        }}
                                        className="w-20 border rounded p-2 text-sm bg-white"
                                    >
                                        {MINUTES.map(m => (
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex gap-2">
                                    {/* Hour */}
                                    <select
                                        value={selectedSlot.end_time.split(":")[0]}
                                        onChange={e => {
                                            const newH = e.target.value;
                                            const oldM = selectedSlot.end_time.split(":")[1];
                                            const newEnd = `${newH}:${oldM}`;
                                            // only allow if >= start
                                            if (newEnd >= selectedSlot.start_time) {
                                                setSelectedSlot(s => ({...s, end_time: newEnd}));
                                            }
                                        }}
                                        className="flex-1 border rounded p-2 text-sm bg-white"
                                    >
                                        {HOURS_RANGE.filter(h => {
                                            const hh = String(h).padStart(2, "0");
                                            return hh >= selectedSlot.start_time.split(":")[0];
                                        }).map(h => {
                                            const hh = String(h).padStart(2, "0");
                                            return <option key={hh} value={hh}>{hh}</option>;
                                        })}
                                    ...
                                    {/* Minute */}
                                    <select
                                        value={selectedSlot.end_time.split(":")[1]}
                                        onChange={e => {
                                            const newM = e.target.value;
                                            const oldH = selectedSlot.end_time.split(":")[0];
                                            const newEnd = `${oldH}:${newM}`;
                                            if (newEnd >= selectedSlot.start_time) {
                                                setSelectedSlot(s => ({...s, end_time: newEnd}));
                                            }
                                        }}
                                        className="w-20 border rounded p-2 text-sm bg-white"
                                    >
                                        {selectedSlot.end_time.split(":")[0] === selectedSlot.start_time.split(":")[0]
                                            ? MINUTES.filter(m => m >= selectedSlot.start_time.split(":")[1])
                                            : MINUTES
                                        }.map(m => (
                                        <option key={m} value={m}>{m}</option>
                                        ))}
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        const updated = selectedLessonStudents.map(s =>
                                                            s.id === stu.id ? {
                                                                ...s,
                                                                progress_evaluation: isNaN(val) ? "" : val
                                                            } : s
                                                        );
